...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if flow_length > args.subvideo_length:
            ...
            for f in range(0, flow_length, args.subvideo_length):
                ...
                pred_flows_bi_sub, _ = fix_flow_complete.forward_bidirect_flow(
                    (gt_flows_bi[0][:, s_f:e_f].to(device), gt_flows_bi[1][:, s_f:e_f].to(device)), 
                    flow_masks[:, s_f:e_f+1].to(device))
        ...
        if video_length > subvideo_length_img_prop:
            ...
            for f in range(0, video_length, subvideo_length_img_prop):
                ...
                prop_imgs_sub, updated_local_masks_sub = model.img_propagation(masked_frames[:, s_f:e_f].to(device), 
                                                                       (pred_flows_bi_sub[0].to(device),pred_flows_bi_sub[1].to(device)), 
                                                                       masks_dilated[:, s_f:e_f].to(device), 
                                                                       'nearest')
                ...
                del prop_imgs_sub, updated_local_masks_sub, pred_flows_bi_sub
                torch.cuda.empty_cache()
                
            updated_frames = torch.cat(updated_frames, dim=1).cpu()
            updated_masks = torch.cat(updated_masks, dim=1).cpu()
            del updated_frames_sub, updated_masks_sub
            torch.cuda.empty_cache()
        else:
            b, t, _, _, _ = masks_dilated.size()
            prop_imgs, updated_local_masks = model.img_propagation(masked_frames.to(device), (pred_flows_bi[0].to(device), pred_flows_bi[1].to(device)), masks_dilated.to(device), 'nearest')
    ...
    for f in tqdm(range(0, video_length, neighbor_stride)):
        ...
        with torch.no_grad():
            ...
            pred_img = model(selected_imgs.to(device), 
                            (selected_pred_flows_bi[0].to(device),
                            selected_pred_flows_bi[1].to(device)), 
                            selected_masks.to(device), 
                            selected_update_masks.to(device), l_t)
            ...
            del selected_imgs, selected_pred_flows_bi, selected_masks, selected_update_masks
            torch.cuda.empty_cache()
